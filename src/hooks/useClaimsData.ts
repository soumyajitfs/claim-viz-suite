    ...
    async function loadData() {
      try {
        ...
        const lines: LineData[] = lineRaw.map((row) => ({
          ...
          lineNum: parseInt(String(row['lineNum'] || 0)),
          seqNum: parseInt(String(row['seqNum'] || 0)),
          coinsAmt: parseFloat(String(row['coinsAmt'] || 0)),
          allowAmt: parseFloat(String(row['allowAmt'] || 0)),
          copayAmt: parseFloat(String(row['copayAmt'] || 0)),
          beginDt: String(row['beginDt'] || ''),
          endDt: String(row['endDt'] || ''),
          revCd: String(row['revCd'] || ''),
          procCd: String(row['procCd'] || ''),
          diagCd: String(row['diagCd'] || ''),
          catCd: String(row['catCd'] || ''),
          qty: parseInt(String(row['qty'] || 0)),
        }));
